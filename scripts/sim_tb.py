...
from scripts.compress import Circuit, Variable, Operation


class NewCircuit:
    OP_MAP = {
        Operation.XOR: operator.__xor__,
        Operation.XNOR: lambda x, y: not (x ^ y),
        Operation.AND: operator.__and__,
        Operation.NOT: operator.__not__,
    }

    def __init__(self, fname: str):
        with open(fname, 'r') as f:
            s = f.read()
        self.circuit = Circuit.from_circuit_str(s)

    def evaluate(self, inputs: dict[Variable, bool]) -> dict[Variable, int]:
        res = {var: inputs[var] for var in self.circuit.inputs}
        for var, computations in self.circuit.computations.items():
            for c in computations:
                res[var] = self.OP_MAP[c.operation](*(res[op] for op in c.operands))
        return res
...
class DutWrapper:
    ...
    def __init__(self, circuit, dut):
        self.dut = dut
        self.rnd_handles = { int(name[3:]): getattr(dut, name) for name in sorted(dir(dut)) if name.startswith("rnd") }
        self.input_handles = { name: getattr(dut, name) for name in sorted(circuit.circuit.inputs, key=lambda x: x[1:]) }
        self.output_handles = { name: getattr(dut, name) for name in sorted(circuit.circuit.outputs, key=lambda x: x[1:]) }
    ...
    def random_pattern(self):
        return (
                {name: random.getrandbits(hnd.value.n_bits) for name, hnd in self.input_handles.items()},
                {name: random.getrandbits(hnd.value.n_bits) for name, hnd in self.rnd_handles.items()},
                )

    def exhaustive_patterns(self):
        def named_patterns(handles):
            for pattern in it.product(*(range(2**hnd.value.n_bits) for hnd in handles.values())):
                yield dict(zip(handles.keys(), pattern))
        yield from it.product(named_patterns(self.input_handles), named_patterns(self.rnd_handles))
    ...
    def outputs_unmasked(self):
        return {name: self.unmask(hnd.value) for name, hnd in self.output_handles.items()}
